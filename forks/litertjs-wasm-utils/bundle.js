...
// src/wasm_loader.ts
var createWasmLib = async (constructorFcn, wasmLoaderScriptOrOptions, assetLoaderScript, glCanvas, fileLocator) => {
  let wasmLoaderScript = wasmLoaderScriptOrOptions;
  let moduleFactory;
  if (typeof wasmLoaderScriptOrOptions === "object" && wasmLoaderScriptOrOptions !== null) {
  ...
  if (moduleFactory) {
    const module2 = await moduleFactory(fileLocator);
    return new constructorFcn(module2, glCanvas);
  }
  if (wasmLoaderScript) {
