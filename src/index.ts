...
function resolveThreadSetting(requested: boolean | undefined): boolean {
...
async function ensureLiteRtLoaded(wasmRoot: string, threads: boolean): Promise<unknown> {
...
export async function createNoiseSuppressionModule(
  ...
  const liteRtWasmRoot =
    options.liteRtWasmRoot ?? resolveAssetUrl(DEFAULT_LITERT_WASM_ROOT);
  const model1Url = options.model1Url ?? resolveAssetUrl(DEFAULT_MODEL1_URL);
  const model2Url = options.model2Url ?? resolveAssetUrl(DEFAULT_MODEL2_URL);
